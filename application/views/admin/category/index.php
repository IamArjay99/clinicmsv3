...
<script>
    ...
    $(document).ready(function() {
        ...
        function tableContent() {
            ...
            let tbodyHTML = '';
            let data = getTableData(`category WHERE is_deleted = 0`);
            data.map((item, index) => {
                let {
                    category_id  = "",
                    name         = "",
                    ...
                    <td>
                        <div class="text-center">
                            <button class="btn btn-outline-info btnEdit"
                                categoryID="${category_id}"><i class="fas fa-pencil-alt"></i></button>
                            <button class="btn btn-outline-danger btnDelete"
                                categoryID="${category_id}"><i class="fas fa-trash-alt"></i></button>
                        </div>
                    </td>
                </tr>`;
            });
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-md-8 col-sm-12 text-right">
                            <button class="btn btn-primary"
                                id="btnAdd"><i class="fas fa-plus"></i> Add Category</button>
                        </div>
                    </div>
                </div>
        ...
        $(document).on("click", "#btnAdd", function() {
            ...
            $("#modal .page-title").text("ADD CATEGORY");
            $("#modal").modal('show');
        ...
        $(document).on("click", ".btnEdit", function() {
            ...
            let data = getTableData(`category WHERE category_id = ${categoryID}`);

            $("#modal .modal-dialog").removeClass("modal-md").addClass("modal-md");
            $("#modal_content").html(preloader);
            $("#modal .page-title").text("EDIT CATEGORY");
            $("#modal").modal('show');
        ...
        $(document).on("click", `#btnSave`, function() {
            ...
            if (validate) {
                ...
                sweetAlertConfirmation("add", "Category", "modal", null, data, true, refreshTableContent);
            }
        })
        // ----- END BUTTON SAVE -----
        ...
        $(document).on("click", `#btnUpdate`, function() {
            ...
            if (validate) {
                ...
                let data = getFormData("modal");
                    ...
                    data["feedback"]    = $(`[name="name"]`).val();
                    data["method"]      = "update";
                    data["whereFilter"] = `category_id=${categoryID}`;
    
                sweetAlertConfirmation("update", "Category", "modal", null, data, true, refreshTableContent);
            }
        })
        // ----- END BUTTON SAVE -----
        ...
        $(document).on("click", `.btnDelete`, function() {
            ...
            sweetAlertConfirmation("delete", "Category", "modal", null, data, true, refreshTableContent);
        })
        // ----- END BUTTON DELETE -----
